...
class TestRetrievalStack:
    ...
    def test_creates_lambda(self, template):
        ...
        template.has_resource_properties(lambda_.CfnFunction.CFN_RESOURCE_TYPE_NAME, {
    ...
    def test_adds_model_invoke_policy_to_lambda_role(self, template):
        expected_props = {
            ...
            "Roles": Match.array_with([
                Match.object_like({"Ref": Match.string_like_regexp(r"RetrievalRole[A-Z0-9]+")})
            ])
        }
        template.has_resource_properties(iam.CfnPolicy.CFN_RESOURCE_TYPE_NAME, expected_props)

    @pytest.mark.it("creates a gateway")
    def test_creates_gateway(self, template):
        template.resource_count_is(bedrockagentcore.CfnGateway.CFN_RESOURCE_TYPE_NAME, 1)
        gateways = template.find_resources(bedrockagentcore.CfnGateway.CFN_RESOURCE_TYPE_NAME)
                        ...
                        "Lambda": Match.object_like({
                            "LambdaArn": Match.object_like({
                                "Fn::GetAtt": Match.array_with([
                                    Match.string_like_regexp(r"RetrievalLambda[A-Z0-9]+"),
                                ])
                            })
                        })
                    })
